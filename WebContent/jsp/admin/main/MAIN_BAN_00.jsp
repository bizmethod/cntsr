...
<div class="wrap_table" >
	...
	<tr>
		<td colspan="6" align="center" >등록된 메인 배너가 없습니다.</td>
	</tr>
	</c:if>
	...
	<tr style="cursor: pointer;" onclick="goDetail('${list.ID_MAIN_IMAGE}')" class="tr_list" seq="<c:out value="${list.ID_MAIN_IMAGE}" />">
		...
		<td>
			<c:if test="${list.LANG_TYP_KR == '1'}" >한</c:if>
			<c:if test="${list.LANG_TYP_EN == '1'}" >영</c:if>
			<c:if test="${list.LANG_TYP_CN == '1'}" >중</c:if>
			<c:if test="${list.LANG_TYP_JP == '1'}" >일</c:if>
		</td>
...
<script>
	...
	var success_function = function(xmlData) {
		GoPage("${pageContext.request.contextPath}/admin/MAIN_BAN_00/default", null, '${TMENUID}', '${SMENUID}', '${METYPE}');
	};
