...
<div class="wrap_table" >
	...
	<tbody>
	<tr>
		<th class="th_first">소송종류</th>
		<td>
			<span>
				<input type="text" class="input_text" id="gubn_nm" name="gubn_nm" style="width:100%;" value="${map.GUBN_NM}" maxlength="10"/>
			</span>
		</td>
	</tr>
	<tr>
		<th class="th_first">수임현황</th>
		<td>
			<span>
	...
	<tr>
		<th class="th_first">소송건수</th>
		<td>
			<span>
				<input type="text" class="input_text" id="law_cnt" name="law_cnt" style="width:100%;" value="${map.LAW_CNT}" maxlength="5"/>
			</span>
		</td>
	</tr>
	<tr>
		<th class="th_first">비고</th>
		<td>
			<span>
				<input type="text" class="input_text" id="note" name="note" style="width:100px;" value="${map.NOTE}" maxlength="250"/>
			</span>
		</td>
	</tr>
<!-- 	<tr> -->
...
function DelEtc(){
	...
	GetAjaxXml('${pageContext.request.contextPath}/admin/ETHI_ETC_03/DelEtc', $("#frmPage").serialize(), success_function);
}


function GoList(){
	var url = "${pageContext.request.contextPath}/admin/ETHI_ETC_02/default";
...
$('#attach_file').change(function() {
	$("#fileYn").val("Y");
	if($("#attach_file").val() != ""){
		GetAjaxXmlMultiPart("frmPage", "${pageContext.request.contextPath}/admin/ETHI_ETC_03/RegAttachFile", null, file_success_function, true, null, 'attach_file', 'gif, ppt, pptx, xsl, xlsx, doc, docx, pdf, hwp, jpg, png, mp3, mp4, zip, avi', true);
	}
});
//첨부파일 등록 성공시
...
function RegData(IU){
	...
	GetAjaxXml('${pageContext.request.contextPath}/admin/ETHI_ETC_03/RegData', data, success_function);
}

success_function = function(){
	GoPage('${pageContext.request.contextPath}/admin/ETHI_ETC_02/default', null ,'${TMENUID}', '${SMENUID}', '${METYPE}');
};
