<%--
  - Author      : 김현우
  - Date        : 2014-07-08
  - Description : 윤리경영 > 반부패청렴 > 예산낭비 신고센터 > 리스트
  - Edited      : 
--%>
...
<div class="top_area">
	...
	<li class="ml10">
			<select name="sel_typ" id="sel_typ" class="sel_basic" onchange="PageReload()">
				<option value="BR" <c:if test="${sel_typ =='BR'}" >selected="selected"</c:if>>예산신고</option>
				<option value="BP" <c:if test="${sel_typ =='BP'}" >selected="selected"</c:if>>예산제안</option>
			</select>
		</li>
	<li class="ml10">
...
// 	alert($("#start_d").val());
	...
	var url = "${pageContext.request.contextPath}/admin/ETHI_REP_00/default";
...
function goDetail(seq){
	...
	var url = "${pageContext.request.contextPath}/admin/ETHI_REP_01/default";
	var param = {seq:seq, sel_typ:$("#sel_typ").val(), TMENUID:'${TMENUID}',SMENUID:'${SMENUID}'}
	GoPage(url, param, '${TMENUID}', '${SMENUID}', '${METYPE}');
}
</script>
