<%--
  - Author      : 김현우
  - Date        : 2014-07-24
  - Description : 시스템관리 > 재무현황 > 자본변동표 > 리스트
  - Edited      : 
--%>
...
<div class="wrap_table" >
	...
	<table class="table_list">
		...
		<tbody>
		<c:if test="${eq_list.size() == 0}">
		<tr>
			<td colspan="4" align="center" >등록된 자본변동표가 없습니다.</td>
		</tr>
		</c:if>
		<c:forEach var="list" items="${eq_list}">
		<tr style="cursor: pointer;" onclick="goDetail('${list.SEQ_CHANGES_EQU}','U')">
			<td class="th_first" onclick="event.cancelBubble = true;"><input type="checkbox" name="chk_bs" <c:if test="${list.DISP_YN == 'Y'}">checked="checked"</c:if> disp_seq="${list.SEQ_CHANGES_EQU}"></td>
...
function goSave(){
	...
	GetAjaxXml('${pageContext.request.contextPath}/admin/DISC_EQU_00/goSave', data, success_function);
}
...
function goDetail(seq,IU){
	...
	var url = "${pageContext.request.contextPath}/admin/DISC_EQU_01/default";
