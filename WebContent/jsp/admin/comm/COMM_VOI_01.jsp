<%--
  - Author      : 김현우
  - Date        : 2014-07-14
  - Description : GKL소개 > 고객광장 > 고객의소리 > 상세
...
<table class="table_detail" style="margin-top: 30px;">
	...
	<tr>
		<th class="th_first"><span>진행통보방식</span></th>
		<td>
			<span>
			<select name="sel_means" id="sel_means" class="sel_basic">
			<c:forEach var="list" items="${pList2}" varStatus="status">
				<option value="${list.CODE}" <c:if test="${map.ANSW_TYP==list.CODE}" >selected="selected"</c:if>>${list.CODE_NM}</option>
			</c:forEach>
			</select>
...
function GoList(){
	var url = "${pageContext.request.contextPath}/admin/COMM_VOI_00/default";
...
success_function = function(){
	GoPage('${pageContext.request.contextPath}/admin/COMM_VOI_00/default', null ,'${TMENUID}', '${SMENUID}', '${METYPE}');
};
