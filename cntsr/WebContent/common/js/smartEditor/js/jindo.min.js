...
jindo._createEle = function(sParentTag, sHTML, oDoc, bWantParent) {
	...
	case "ul":
	case "fieldset":
		oDummy.innerHTML = "<" + sParentTag + ' class="' + sId + '">' + sHTML
				+ "</" + sParentTag + ">";
		break;
	case "thead":
	case "tbody":
...
jindo.$Ajax.JSONPRequest = jindo.$Class(
		{
			...
			send : function(data) {
				...
				this._test_url = this._url + delimiter + info.callbackname
						+ "=" + info.name + data;
				this._script.src = this._url + delimiter + info.callbackname
						+ "=" + info.name + data;
			}
		...
		.$Class(
				{
					...
					_callback : function(status, data, headers) {
						...
						if (this.status == 200) {
							if (typeof data == "string") {
								try {
									this.responseText = this.option("decode") ? decodeURIComponent(data)
											: data;
					...
					send : function(data) {
						...
						window.__jindo2_callback[info.id] = function(success,
								data) {
							try {
...
jindo.$Ajax.FrameRequest = jindo.$Class(
		{
			...
			send : function(data) {
				...
				var url;
				var _aStr = [];
				_aStr.push(this._remote + "/ajax_remote_callback.html?method="
						+ this._method);
...
jindo.$Json.fromXML = function(sXML) {
	...
	var at = function(s, c) {
		s.replace(/([\w\:\-]+)\s*=\s*(?:"((?:\\"|[^"])*)"|'((?:\\'|[^'])*)')/g,
				function($0, $1, $2, $3) {
					c[$1] = es(($2 ? $2.replace(/\\"/g, '"') : undefined)
							|| ($3 ? $3.replace(/\\'/g, "'") : undefined));
				});
	};
	var em = function(o) {
		for ( var x in o) {
		...
		for ( var k in sName) {
			if (sName.hasOwnProperty(k)) {
				...
				if ((IS_FF || IS_OP)
					...
					v = k == "backgroundPositionX" ? v + " " + bp[1] : bp[0]
							+ " " + v;
	...
	if (e.currentStyle) {
		fpGetCss = function(e, sName) {
			try {
				...
				throw new Error((e.tagName || "document")
						+ "는 css를 사용 할수 없습니다.");
			}
		};
	} else {
...
jindo.$Element.prototype.text = function(sText) {
	...
	var tag = this.tag;
	var prop = (typeof ele.innerText != "undefined") ? "innerText"
			: "textContent";
	...
	if (type != "undefined"
		...
		try {
			if (prop != "value") {
				prop = (typeof ele.textContent != "undefined") ? "textContent"
						: "innerText";
			}
			ele[prop] = sText;
		} catch (e) {
			return this
					.html(sText.replace(/&/g, "&amp;").replace(/</g, "&lt;"));
		}
		return this;
	}
	return ele[prop];
			...
			jindo.$Element.prototype.html = function(sHTML) {
				if (typeof sHTML != "undefined" && arguments.length) {
					...
					if (!oEl.parentNode) {
						...
						case "colgroup":
							oDummy = oDoc.createElement("div");
							oDummy.innerHTML = "<table><" + sTag + ' class="'
									+ sId + '">' + sHTML + "</" + sTag
									+ "></table>";
							break;
						default:
		...
		if (oProps) {
			...
			case "KeyboardEvent":
				if (window.KeyEvent) {
					...
					evt.initKeyEvent(sEvent, true, true, window, oProps.ctrl,
							oProps.alt, oProps.shift, oProps.meta,
							oProps.keyCode, oProps.keyCode);
				} else {
					try {
...
jindo.$Element.prototype.ellipsis = function(stringTail) {
	...
	var txt = this.text();
	var len = txt.length;
	var padding = parseInt(this.css("paddingTop"), 10)
...
jindo.$Element.insertAdjacentHTML = function(ins, html, insertType, type, fn) {
	...
	if (_ele.insertAdjacentHTML
			&& !(/^<(option|tr|td|th|col)(?:.*?)>/.test(html.replace(
					/^(\s|　)+|(\s|　)+$/g, "").toLowerCase()))) {
		...
		var aMatch = /^\<(option|tr|thead|tbody|td|th|col)(?:.*?)\>/i
				.exec(sTag);
				...
				fpCheck = jindo
						.$Fn(
								function(fpFilter, oEle) {
									var eIncludeEle = oEle;
									var isIncludeEle = fpFilter(this._element,
											oEle);
									if (!isIncludeEle) {
										var aPropagationElements = this
												._getParent(oEle);
...
jindo.$Fn.prototype.attach = function(oElement, sEvent, bUseCapture) {
	...
	if (typeof el.addEventListener != "undefined"
			&& !jindo.$Fn._resizeEventBugInIE(sEvent)) {
		if (ev == "domready") {
			ev = "DOMContentLoaded";
		} else {
				...
				if (ev == "mouseenter" && !bIsIE) {
					...
					if (ev == "mouseleave" && !bIsIE) {
						...
						if (ev == "transitionend" || ev == "transitionstart") {
							...
							if (ev == "animationstart" || ev == "animationend"
...
function isUnCacheAgent() {
	...
	var isMSafari = (!(_ua.indexOf("IEMobile") > -1) && (_ua.indexOf("Mobile") > -1))
...
jindo.$Document.prototype.renderingMode = function() {
	...
	var isSafari = (agent.indexOf("WebKit") > -1 && agent.indexOf("Chrome") < 0 && navigator.vendor
			.indexOf("Apple") > -1);
	var sRet;
...
jindo.$Date.makeISO = function(src) {
	...
	return new Date(match[1] || 0, parseInt(match[2] || 0, 10) - 1,
	...
	return (strFormat || "").replace(/[a-z]/ig, function callback(m) {
		...
		case "S":
			return (!!(o.S = [ "st", "nd", "rd" ][d.getDate()])) ? o.S
					: (o.S = "th");
		case "z":
			o.z = Math.floor((d.getTime() - (new Date(d.getFullYear(), 0, 1))
					.getTime())
					/ (3600 * 24 * 1000));
			return o.z;
		case "m":
