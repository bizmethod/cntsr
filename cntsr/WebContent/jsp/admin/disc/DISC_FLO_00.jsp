<%--
  - Author      : 김현우
  - Date        : 2014-07-24
  - Description : 시스템관리 > 재무현황 > 현금흐름표 > 리스트
  - Edited      : 
--%>
...
<div class="wrap_table" >
	...
	<table class="table_list">
		...
		<tbody>
		<c:if test="${flow_list.size() == 0}">
		<tr>
			<td colspan="4" align="center" >등록된 현금흐름표가 없습니다.</td>
		</tr>
		</c:if>
		<c:forEach var="list" items="${flow_list}">
		<tr style="cursor: pointer;" onclick="goDetail('${list.SEQ_CASH_FLOW}','U')">
			<td class="th_first" onclick="event.cancelBubble = true;"><input type="checkbox" name="chk_bs" <c:if test="${list.DISP_YN == 'Y'}">checked="checked"</c:if> disp_seq="${list.SEQ_CASH_FLOW}"></td>
...
function goSave(){
	...
	GetAjaxXml('${pageContext.request.contextPath}/admin/DISC_FLO_00/goSave', data, success_function);
}
...
function goDetail(seq,IU){
	...
	var url = "${pageContext.request.contextPath}/admin/DISC_FLO_01/default";
